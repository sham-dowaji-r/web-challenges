...
export default function App() {
  const [selectHabitats, setSelectHabitats] = useState("");
  const selectedAnimals = animals.filter(
    (animal) => selectHabitats === "" || animal.habitat === selectHabitats
  );
  return (
    <div>
      {habitats.map((habitat) => (
        <button
          className={selectHabitats === habitat ? "selected" : "button"}
          key={habitat.id}
          onClick={() => setSelectHabitats(habitat)}
        >
          {habitat}
        </button>
      ))}
      <button
        className={selectHabitats === "" ? "selected" : "button"}
        onClick={() => setSelectHabitats("")}
      >
        Show All
      </button>
      <ul>
